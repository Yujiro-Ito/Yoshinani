...
| romaji→kana | **自前の決定的変換** | モデル不要。テーブル＋促音/撥音ルール |
| kana→kanji | **zenz (Miwa-Keita/zenz, GGUF)** | 90M・変換専用・CC-BY-SA 4.0 |
| LLM実行基盤 | **llama.cpp** | zenzのGGUFを直接実行。Androidでも動く |
| 学習・体感用 | Ollama | 「ローカルLLM実行」を初めて触る練習用。本番には使わない |
...
> 更新(2026-06-09): **確定トリガーは `Space`（既定）** に変更。トリガーキーは
> **設定（`settings.json`）で変更・追加できる**ようにした（[[phase1-trigger-commit]]）。
> `。`/`、`/`Shift+Tab` は当初トリガーだったが撤去し、設定で再追加できる位置づけ。

| キー | 役割 |
|--|--|
| `Space`（既定トリガー） | 変換 + 自動確定（トリガーキー自体は消費）。preedit が空なら通常の空白として素通し |
| `Backspace` | preedit末尾を削除 |
| `Esc` | preedit取り消し |
| ~~`Enter`~~ | **使わない**（A自動確定では不要） |
| （設定で追加可）`。` `、` `Shift+Tab` 等 | キーマップ設定で確定トリガーに追加できる |

```
打鍵中 ──→ Space（既定トリガー）──→ 変換+自動確定+手放す
   │                                   │
   └─ （。、等もトリガーに追加可・設定）  すぐ次の入力へ（止まらない）
```

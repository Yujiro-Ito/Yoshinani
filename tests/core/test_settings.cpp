...
TEST_CASE("conversionOnKeys / directOnKeys: 既定は空・JSON から読める・ラウンドトリップ") {
    auto d = ParseSettings("");
    CHECK(d.conversionOnKeys.empty());
    CHECK(d.directOnKeys.empty());

    auto s = ParseSettings(R"({
        "conversionOnKeys": ["NonConvert", "Convert"],
        "directOnKeys":     ["Capital"]
    })");
    REQUIRE(s.conversionOnKeys.size() == 2);
    CHECK(s.conversionOnKeys[0] == "NonConvert");
    REQUIRE(s.directOnKeys.size() == 1);
    CHECK(s.directOnKeys[0]     == "Capital");

    Settings src{};
    src.conversionOnKeys = {"NonConvert"};
    src.directOnKeys     = {"Convert"};
    auto round = ParseSettings(SerializeSettings(src));
    CHECK(round.conversionOnKeys == src.conversionOnKeys);
    CHECK(round.directOnKeys     == src.directOnKeys);
}
